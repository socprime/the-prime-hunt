...
describe('ArcSight App tests', () => {
  const platformID = PlatformID.ArcSight;
  ...
  test('should be rendered', () => {
    ...
    expect(screen.getByText(`${platform.getName()} detected. Run a query to see results.`)).toBeInTheDocument();
  });
  ...
  test('should provide copy actions', async () => {
    ...
    expect(mockedCopyToClipboard.mock.calls[0][0]).toEqual('AND test0 = "account1"');
    ...
    expect(mockedCopyToClipboard.mock.calls[1][0]).toEqual('AND (test0 = "account1" OR test0 = 2 OR test0 = "test2" OR test0 = 4 OR test0 = "test\\domain.com" OR test0 = "\\"process.exe\\" -a -g https://some.site/some-page")');
  });
